...
export default function CuisineEtape1Screen({ navigation}) {
 ...
 for (let x=0 ; x < recette.length; x++) {
  ...
  fetch(`${BACKEND_ADDRESS}menuTer/miseenoeuvre?recettesList=[${titleList}]`)
    ...
    .then((data) => {
      for (let x=0 ; x < data.steps.assemblage.length; x++) {
      const Recipe = {
        action: data.steps.assemblage[x].step.action,
        duration: data.steps.assemblage[x].step.duration,
        title: data.steps.assemblage[x].recette_title,
      }
    ...
    <ScrollView style={styles.container}>
      <Menu  />
         <View style={styles.generalContainer}>
             ...
             <Text style={styles.title}>Assemblage </Text>
        {renderSteps}
